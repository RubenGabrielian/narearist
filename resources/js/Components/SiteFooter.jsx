...
export default function SiteFooter() {
    ...
    const handleEmailSubmit = (event) => {
        ...
        if (!emailForm.data.email.trim()) return;
    ...
    return (
        <footer className="relative bg-[#DB3106] text-white py-10 sm:py-12 overflow-hidden">
            <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-20">
                <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
                    <div className="flex flex-col gap-8 md:gap-10 lg:flex-row lg:items-start">
                        <div>
                            <nav className="space-y-4 sm:space-y-5 text-center lg:text-left">
                                <a href="/" className="block hover:text-yellow-200 transition-colors text-sm sm:text-base">
                                    Գլխավոր
                                </a>
                                <a href="/about-author" className="block hover:text-yellow-200 transition-colors text-sm sm:text-base">
                                    Հեղինակի մասին
                                </a>
                                <a href="/gallery" className="block hover:text-yellow-200 transition-colors text-sm sm:text-base">
                                    Պատկերասրահ
                                </a>
                                <a href="https://t.me/tennisacademynovel" className="block hover:text-yellow-200 transition-colors text-sm sm:text-base">
                                    Քննարկումների սենյակ
                                </a>
                                <a href="/about-author#contact" className="block hover:text-yellow-200 transition-colors text-sm sm:text-base">
                                    Կապ
                                </a>
                            </nav>
                        </div>
                        ...
                        <div className="w-full flex justify-center lg:justify-start">
                            <div className="flex flex-col w-full max-w-[560px]">
                                <form onSubmit={handleEmailSubmit} className="w-full">
                                    <label htmlFor="" className="mb-4 sm:mb-5 block text-center lg:text-left text-sm sm:text-base">
                                        Գրանցվի՛ր, որ նորություններից հետ չմնաս
                                    </label>
                                    <div className="relative flex items-center">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-5 sm:px-6 py-3 pr-12 rounded-full bg-white backdrop-blur-sm border border-white/20 text-black placeholder-black/20 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-white/50 transition-all placeholder:text-sm"
                                        />
                                        <button
                                            type="submit"
                                            disabled={emailForm.processing}
                                            className="absolute right-2 bg-black/10 transition-colors rounded-full w-[40px] h-[40px] hover:bg-white/30 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center"
                                    ...
                                    {emailForm.processing && (
                                        <p className="mt-2 text-sm text-white">Պահպանվում է...</p>
                                    )}
                                    {emailForm.errors.email && (
                                        <p className="mt-2 text-sm text-white">{emailForm.errors.email}</p>
                                    )}
                                    {emailForm.recentlySuccessful && (
                                        <p className="mt-2 text-sm text-white">Շնորհակալություն Թենիսի Ակադեմիային միանալու համար</p>
                                    )}
                                </form>
                                <div className="mt-8 sm:mt-10 flex justify-center lg:justify-between gap-2 sm:gap-3 flex-wrap">
                    ...
                    <div className="flex justify-center lg:justify-end">
                        <div className="relative">
                            <div className="w-32 h-32 sm:w-40 sm:h-40 md:w-52 md:h-52 relative">
                                <div
                                    className="absolute cursor-pointer inset-0 transition-all duration-1000 ease-out"
                                    style={{
                                        transform: `translate(${ballPosition.x}px, ${ballPosition.y}px) rotate(${ballPosition.rotation}deg)`,
                                    }}
                                >
                                    <a href="https://ko-fi.com/tennisacademy" className='decoration-none' style={{ textDecoration: 'none', outline: 'none' }} target='_blank'>
                                        <img
            ...
            <p className="text-xs sm:text-sm opacity-90 max-w-8xl mx-auto px-4 sm:px-6 lg:px-20 mt-6 text-center lg:text-left">
                Copyright ©  {new Date().getFullYear()}  Naré Arist. All rights reserved.
            </p>
        </footer >
    );
}
